...
type node = { nb_depend: int ; neighbours : Graph.neighbours ; color : color }
...
let build_dependency_from (g:Graph.t) (label:Graph.nodeLabel) : gdepend =
  ...
  let neigh =
    try Graph.get_neighbours g label
    with Graph.NonExistingNode -> failwith "Dependency.build_dependency_from : Non existing first node."
  in
  ...
  and  traversal gdepend neigh =
    Graph.fold_neighbours build neigh gdepend
  in
