...
let region_to_set ({r=rmin;c=cmin}, {r=rmax;c=cmax}) =
  let rec add_one_row s pos =
    if pos.c > cmax then s
    else
      let s = Spos.add pos s in
      add_one_row s {pos with c=pos.c+1} in
...
type node = { content : node_content ; neighbours : neighbours }
...
let add_neighbour g label new_neighbour =
  ...
  match node_opt with
  | None ->
    Mpos.add new_neighbour {content = Val Undefined ; neighbours = label++empty_neighbours} g
...
let add_node g label ({content;neighbours} as node) =
  ...
  let g = match existing_node_opt with
...
let change_node g label ({content;neighbours} as node) =
  ...
  let g = match existing_node_opt with
    ...
    | Some {content = Occ (region,_) ; neighbours = old_neighbours } ->
      ...
      let g = fold_neighbours
          (fun lab gg ->
             match Mpos.find_opt lab gg with
             | None -> failwith "Graph.change_node : Non dependencies when we should have one."
             | Some nl ->
               let new_neigh = remove_neighbours label nl.neighbours in
               if new_neigh = empty_neighbours then
          ...
          g
      in Mpos.add label {content; neighbours = neighbours @@ old_neighbours} g
  in
  add_neighbours_ g label node
...
let content_to_string content = match content with
  | Val v -> value_to_string v
  | Occ ((p1, p2), v) -> "Occ ("^(pos_to_string p1)^", "^(pos_to_string p2)^"), "^value_to_string v
...
let print g =
  ...
  List.iter
    (fun (pos, {content; neighbours}) ->
       ...
       print_string ("pos = "^(pos_to_string pos)^"   content = "^cs^"   neighbours = ");
       Spos.iter (fun pos -> let s = pos_to_string pos in print_string (s^" ; ")) neighbours;
       print_endline ""
    ) bind ; print_endline ""
