...
let build_dependency_from_ (g : Graph.t) (label : Graph.nodeLabel) (gdepend:gdepend) =
  let neigh, content =
    try Graph.get_neighbours_content label g with Graph.NonExistingNode ->
      failwith "Dependency.build_dependency_from_ : Non existing first node." in
...
let build_dependency_from_all (g : Graph.t) (formulas : (pos*Ast.content) list): gdepend =
  let rec build_acc gdepend = function
    | [] -> gdepend
    | (label, _) :: tail ->
      match Mpos.find_opt label gdepend with
      | Some {color=Black;_} -> build_acc gdepend tail
      | Some {color=White;_} -> failwith "Dependency.build_dependency_from : WHAT ?"
      | None ->
