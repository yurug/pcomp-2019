 ...
 */
 ...
 extends Request_change(case_toChange, expression){ 
  ...
  def this (c: Case, value : CaseData, sheet: Sheet_evalued) = {
    ...
    /*
     * precondition:
     * case_toChange is a valide case  
     * expression is a valid expression
     * sheet is no null
     * operation:
     * evaluate expression of case case_toChange
     * give all case to change for this new value of case_toChange in sheet
     * postcondition:
     * if succefull execution, this.result contain list of case to change
     * if unsuccefull execution, this.result can contain incomplet list of case to change
     */
     def exec():Unit = { 
   ...
   * sheet is valid
   * postcondition:
   * list de case to change for a new value of 1 formule in sheet
   */
